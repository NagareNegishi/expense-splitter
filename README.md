...
Money is integer cents throughout. Per expense, shares are computed with floor division; leftover cents are handed out one each to non-payers first, so shares always sum to the exact expense amount.

The settle-up list uses a greedy algorithm: match the largest creditor against the largest debtor each step, producing at most n−1 transactions. This is near-minimal in practice but not guaranteed minimal (the true minimum is NP-hard). See `docs/DECISIONS.md` §10 for the full reasoning and worked examples.
...
- Single group, no routing or multi-session support.
